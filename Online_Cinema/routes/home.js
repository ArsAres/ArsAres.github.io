...
function getUserFindString(req){
    req.query.find = (req.query.find) ?
    req.query.find.trim().replace(/\s+/g, "_").toLowerCase().replace(/[&\?\*\.]/g, '') : "";
    return req.query.find.replace(/_/g, " ");
};
exports.get = function(req, res, next) {//кровавый ад
    
    let find=getUserFindString(req); //запрос поиска
    let req_genres = (req.query.genre) ? req.query.genre.split('.') : [];//жанры
    let type = (req.query.type == "movie") ? 1 : (req.query.type == "series") ? 2 : 0;//1-фильм/2-сериал/0-все
    let one_page_count = (req.query.count == "10") ? 10 : (req.query.count == "30") ? 30 :
        (req.query.count == "40") ? 40 : (req.query.count == "50") ? 50 : 20;//количество отображаемых постеров
    let title = (find) ? `Результаты по запросу "${find}"` + ((type) ? (type == 1) ? " - фильмы" : " - сериалы" : "")
        : (!type) ? "Все фильмы и сериалы" : (type == 1) ? "Все фильмы" : "Все сериалы";//заголовок исходя из типа
    let params = {};
