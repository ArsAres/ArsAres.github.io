...
exports.get = function({title, imdb_id, kinopoisk_id}) {
    ...
    let url = "https://videocdn.tv/api/short?api_token=" + config.get('videocdn:user_hash')
        + (kinopoisk_id ? "&kinopoisk_id=" + kinopoisk_id : "&title=" + encodeURI(title));
    ...
    return new Promise(function (resolve, reject) {
        request(url, function (error, response, body) {
            ...
            if (error) return reject(new Error("videocdn: не отвечает!\n(" + error + ")"));

            let data = JSON.parse(body);
            if (data.status == "error") return reject(new Error("videocdn: произошла ошибка!\n(" + data.message + ")"));

            data = data.data;
            if (!data.length) return reject(new Error("videocdn: ничего не найдено, попробуйте поиск по IMDBb id и Кинопоиск id!"));
            ...
            if (!info.kinopoisk_data) return reject(new Error("videocdn: ничего не найдено, попробуйте поиск по IMDBb id и Кинопоиск id!"));
            ...
            request(url, function (error, response, body) {
                ...
                if (info.imdb_data.Response == "False") return reject(new Error("OMDbApi: произошла ошибка!\n(" + info.imdb_data.Error + ")"));
                 info.kinopoisk_data.kinopoisk_id=kinopoisk_id;
                resolve(info);
            });
        });
    });
