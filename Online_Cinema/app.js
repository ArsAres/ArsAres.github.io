...
let upload = multer({ dest: 'public/images/users/' });
...
app.post('/uploadphoto', upload.single('photo'), function (req, res, next) {
    ...
    if (req.file.mimetype != "image/png" && req.file.mimetype != "image/jpeg"&& req.file.mimetype != "image/jpeg")
        return res.status(403).json({message: "Неподдерживаемый тип"});
    console.log('dir: ' + __dirname + "\\" + req.file.path);
    fs.rename(__dirname + "\\public\\images\\users\\" + req.file.filename,__dirname + "\\public\\images\\users\\" + res.locals.user + ".png", (err) => {
        if(err) throw err;
        res.json({message: "Фотография профиля обновлена"});
    });
});
...
app.post('/deletephoto', function (req, res, next) {
    ...
    fs.unlink(__dirname + "\\public\\images\\users\\" + res.locals.user + ".png", function(err) {
        console.log(err);
        res.json({message: "Фотография профиля удалена"});
    });
});
